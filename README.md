...
with the official Weblate catalogs while keeping untranslated UI strings easy to find and fill.

## Start translating

1. Choose the `sync/vX.Y` branch matching your DSW minor release.
2. Open `translations/README.md` on that branch.
3. Choose an English source from the **Open** section and fill its empty `Translation (zh_Hant)` block in
   GitHub.
